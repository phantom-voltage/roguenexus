#!/usr/bin/python
#
# Modfied external score parser for Poschengband. Based
# off of the script by Erik Osheim.
#
# External score parser/printer for Poschengband. It hardcores some
# values from p_races.txt and classes.txt for simplicitly.
# Should basically mimic the built-in format.
#
# Seems to work as of 2017-07-18 (Poschengband v6.1.0).
...
def display(p):
    ...
    prs = personalities.get(p['pers'], 'Unknown')
    ...
    print "%10d  %s the %s %s %s, level %s" % (p['pts'], p['name'], prs, race, cls, clvl)
